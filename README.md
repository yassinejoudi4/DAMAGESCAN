...
L'historique complet d'entraînement (accuracy/loss par epoch) est disponible dans [outputs/training_history.json](outputs/training_history.json)
...
d) Télécharger le dataset avec : Invoke-WebRequest -Uri "https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/ZjXM4RKxlBK9__ZjHBLl5A/aircraft-damage-dataset-v1.tar" -OutFile "data/aircraft-damage-dataset-v1.tar"
tar -xvf data/aircraft-damage-dataset-v1.tar -C data/
e) Exécuter le pipeline complet dans l'ordre :
python src/train.py
...
- Finaliser l'intégration du module de génération de légendes BLIP (basé sur un modèle Transformer), actuellement en cours de développement
- Appliquer de la data augmentation pour réduire l'overfitting et améliorer l'accuracy de validation
- Effectuer un fine-tuning des couches supérieures de VGG16 (dégel partiel) pour une meilleure adaptation des features
- Agrandir le dataset pour une meilleure généralisation
- Déployer le modèle sous forme d'application web (Streamlit ou Flask) pour permettre de l'inférence interactive

---

## 8. Dataset

Le dataset utilisé provient de [Aircraft Damage Dataset](https://universe.roboflow.com/youssef-donia-fhktl/aircraft-damage-detection-1j9qk), fourni via Roboflow, sous licence CC BY 4.0.
